...
module Alcotest = struct
  ...
  let int32 =
    ...
    (module M : TESTABLE with type t = int32)
  ...
  let int64 =
    ...
    (module M : TESTABLE with type t = int64)

  let float =
    let module M = struct
      type t = float
      let pp fmt i = Format.pp_print_string fmt (string_of_float i)
      let equal (a : float) (b : float) = compare a b = 0
    end in
    (module M : TESTABLE with type t = float)
end
...
module Endian(Es : EndianString.EndianStringSig) = struct
  ...
  let make_tests e parse = e.name, `Quick, begin fun () ->
    check_ok ~msg:"zero"     e.test parse [to_string e e.zero]          e.zero;
    check_ok ~msg:"min"      e.test parse [to_string e e.min]           e.min;
    check_ok ~msg:"max"      e.test parse [to_string e e.max]           e.max;
    check_ok ~msg:"trailing" e.test parse [to_string e e.zero ^ "\xff"] e.zero;
  end
